...
check('landing title', (await page.title()).includes('Heartland Bio Labs'), 'true');
check('landing service cards', await page.locator('.svc').count(), 5);
check('landing links to quote desk', await page.locator('a[href="quote/"]').count() > 0, 'true');
check('landing FAQ entries', await page.locator('.faq details').count(), 5);
const ld = await page.locator('script[type="application/ld+json"]').innerText();
check('landing JSON-LD parses', (() => { try { JSON.parse(ld); return true; } catch { return false; } })(), 'true');

// ---- Quote desk -------------------------------------------------------------
await page.goto(new URL('./quote/index.html', import.meta.url).href);
await page.waitForTimeout(400);

check('brand is Heartland', await page.locator('#lab-name').innerText(), 'Heartland Bio Labs');
...
check('summary has lab name', summary.includes('HEARTLAND BIO LABS'), 'true');
...
// Persistence across reload — and the draft key must be Heartland's own
check('draft key namespaced',
  await page.evaluate(() => localStorage.getItem('heartland.quote.draft.v1') !== null), 'true');
await page.reload();
await page.waitForTimeout(400);
...
// Mobile layout sanity — quote desk, then the landing page
await page.setViewportSize({ width: 390, height: 844 });
await page.waitForTimeout(150);
const overflow = await page.evaluate(() =>
  document.documentElement.scrollWidth - document.documentElement.clientWidth);
check('no horizontal overflow on mobile (quote)', overflow <= 0, 'true');

await page.goto(new URL('./index.html', import.meta.url).href);
await page.waitForTimeout(200);
const overflowLand = await page.evaluate(() =>
  document.documentElement.scrollWidth - document.documentElement.clientWidth);
check('no horizontal overflow on mobile (landing)', overflowLand <= 0, 'true');
